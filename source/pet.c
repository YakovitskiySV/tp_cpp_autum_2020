...
int create_pet(pet *new_pet,
    ...
    if (set_pets_name(new_pet,
                      expected_name) != 0) {
        return 1;
    }
    if (set_pets_type(new_pet,
                      expected_type) != 0) {
        free(new_pet->name);
        return 1;
    }
    if (set_pets_color(new_pet,
                       expected_color) != 0) {
        free(new_pet->name);
        free(new_pet->type);
        return 1;
...
int print_pets_by_type(pet *pets,
                       size_t *pets_number,
    ...
    for (size_t i = 0 ; i < *pets_number ; ++i) {
        ...
        if (strcmp(pets[i].type,
                    requested_pets_type) == 0) {
            print_pet(&pets[i]);
            ++counter;
        }
    }
    if (counter == 0) {
        printf("there are no animal with type %s :-(",
               requested_pets_type);
    }
    return 0;
}
